...
function loadBagItemObjects() {
  ...
  bag_items_objects= bagItems.map(itemId => {
...
function display_bag_items() {
  ...
  let innerHTML = '';
  bag_items_objects.forEach(bagItem => {
    innerHTML += generate_html(bagItem);
  });
...
function displayBagSummary(){
  ...
  let totalItems=bag_items_objects.length
  let totalPrice=0;
  let discountPrice=0;
  bag_items_objects.forEach(bagItem=>{
  ...
  let totalAmount=totalPrice-discountPrice+99;
